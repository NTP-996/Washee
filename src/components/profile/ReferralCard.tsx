...
export default function ReferralCard() {
  ...
  return (
    <section className="rounded-2xl border border-hairline bg-panel p-6">
      <h2 className="mb-1 text-sm uppercase tracking-widest text-muted">{t('referral.title')}</h2>
      <p className="mb-4 text-sm text-muted">{t('referral.subtitle')}</p>
        ...
        {ref && ref.coupons.length > 0 ? (
          <ul className="space-y-2">
            {ref.coupons.map((c) => (
              ...
              >
                <span>{tf('percentOff', { percent: c.discountPercent })}</span>
                <span className="text-muted">{t(`coupon.${c.status}`)}</span>
              </li>
            ))}
          </ul>
        ) : (
          <p className="text-sm text-muted">{t('referral.empty')}</p>
        )}
      </div>
    </section>
  );
