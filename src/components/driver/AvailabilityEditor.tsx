...
export default function AvailabilityEditor() {
    ...
    try {
      setSlots(
        await api<CalendarSlot[]>(
          `/api/driver/slots?from=${todayISO()}&to=${addDaysISO(todayISO(), 180)}`,
          { driver: true },
        ),
      );
    } catch {
  ...
  async function add(e: FormEvent): Promise<void> {
    ...
    try {
      ...
      // 409 = duplicate time on this day.
      setError(err instanceof ApiRequestError ? err.message : 'Failed to add slot');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function remove(id: string): Promise<void> {
    setError('');
    try {
      await api(`/api/driver/slots/${id}`, { driver: true, method: 'DELETE' });
    } catch (err) {
    ...
    <section className="relative rounded-2xl border border-hairline bg-panel p-5 sm:p-6">
      ...
      <div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_18rem]">
        ...
        <div className="mt-6 border-t border-hairline pt-6 md:mt-0 md:border-l md:border-t-0 md:pl-6 md:pt-0">
          ...
          <form onSubmit={add} className="mb-2 flex items-end gap-2">
